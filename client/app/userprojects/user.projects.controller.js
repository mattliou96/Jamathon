(function () {
    ...
    angular
        .module('Jamathon')
        .controller('userProjectCtrl', userProjectCtrl);

    userProjectCtrl.$inject = ['ProjectSvc', 'AuthFactory', 'ngAudio'];

    function userProjectCtrl(ProjectSvc, AuthFactory,ngAudio) {
        ...
        // function init() {
        //     console.log("init");
        //     ProjectSvc.getAllUsersProjects()
        ...
        // vm.sound = ngAudio.load("sounds/test-project.wav"); // returns NgAudioObject
        ...
        var dest = sequencer.audio.audioContext.createMediaStreamDestination();
        //var mediaRecorder = new MediaRecorder(sequencer.audio.audioContext)
        var splitter = sequencer.audio.audioContext.createChannelSplitter(2);
        var chunks = [];
        vm.buttonPlayState = true;
        vm.buttonStopState = false;
        
        var recorder = new Recorder(splitter);

        vm.buttonHandler = function () {
            console.log(vm.buttonPlayState);
            console.log(vm.buttonStopState);
            
            if (vm.buttonStopState) {
                console.log("stop")
                recorder.stop();
                recorder.exportWAV(function (blob) {
                    document.getElementById("audio").href = URL.createObjectURL(blob);
                    document.getElementById("audio").download = "my Recording.wav";
                })
                vm.buttonPlayState = true;
                vm.buttonStopState = false;
            } 
            else if (vm.buttonPlayState) {
                console.log(recorder);
                console.log("start")
                recorder.record();
                sequencer.audio.playSequence(sequencer.project.state.activeProject.Tracks, 2)
                vm.buttonPlayState = false;
                vm.buttonStopState = true;
            }
        ...
        splitter.connect(sequencer.audio.audioContext.destination, 0);
        splitter.connect(dest, 1);
        sequencer.audio.nodes.masterVolume.connect(splitter);
        ...
        sequencer.init = function () {
            utils.net.loadSequence('app/userprojects/assets/test-project.json', function (response) {
        ...
        //How to create downloadable wav file and link uing blob
